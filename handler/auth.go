...
type setInviteCodeStatusRequest struct {
...
func AdminSetUserStatus(w http.ResponseWriter, r *http.Request, id string) {
	var request setUserStatusRequest
	_ = json.NewDecoder(r.Body).Decode(&request)
	user, err := service.SetUserStatus(id, request.Status)
	if err != nil {
		FailError(w, err)
		return
	}
...
func AdminInviteCodes(w http.ResponseWriter, r *http.Request) {
	result, err := service.ListInviteCodes(parseQuery(r))
	if err != nil {
		FailError(w, err)
		return
	}
...
func AdminGenerateInviteCodes(w http.ResponseWriter, r *http.Request) {
	var request generateInviteCodesRequest
	_ = json.NewDecoder(r.Body).Decode(&request)
	user, _ := service.UserFromContext(r.Context())
	items, err := service.GenerateInviteCodes(user.ID, request.Count, request.Remark)
	if err != nil {
		FailError(w, err)
		return
	}
...
func AdminSetInviteCodeStatus(w http.ResponseWriter, r *http.Request, id string) {
	var request setInviteCodeStatusRequest
	_ = json.NewDecoder(r.Body).Decode(&request)
	item, err := service.SetInviteCodeStatus(id, request.Status)
	if err != nil {
		FailError(w, err)
		return
	}
