...
export type AdminInviteCodeStatus = "unused" | "used" | "disabled";

export type AdminInviteCode = {
    id: string;
    code: string;
    status: AdminInviteCodeStatus;
    createdBy: string;
    usedByUserId: string;
    usedByName: string;
    usedAt: string;
...
export async function setAdminUserStatus(token: string, id: string, status: AdminUser["status"]) {
    return apiPost<AdminUser>(`/api/admin/users/${encodeURIComponent(id)}/status`, { status }, token);
}
...
export async function generateAdminInviteCodes(token: string, count: number, remark = "") {
    return apiPost<AdminInviteCode[]>("/api/admin/invite-codes/generate", { count, remark }, token);
}

export async function setAdminInviteCodeStatus(token: string, id: string, status: AdminInviteCodeStatus) {
    return apiPost<AdminInviteCode>(`/api/admin/invite-codes/${encodeURIComponent(id)}/status`, { status }, token);
}
