    ...
    return (
        ...
        >
            ...
            >
                ...
                {!hasImageContent && !hasVideoContent && !isLayerText ? <div className="pointer-events-none absolute inset-x-0 bottom-0 h-12" style={{ background: `linear-gradient(to top, ${theme.canvas.background}66, transparent)` }} /> : null}
...
function TextContent({ node, theme, isEditingContent, textareaRef, onContentChange, onStopEditing, onGenerateImage }: NodeContentRendererProps) {
    if (node.metadata?.layerText) {
        const textStyle: React.CSSProperties = {
            ...
            fontFamily: node.metadata.fontFamily || "sans-serif",
            fontWeight: (node.metadata.fontWeight || "normal") as React.CSSProperties["fontWeight"],
        ...
        if (isEditingContent) {
            return (
                <textarea
                    ref={textareaRef}
                    className="block h-full w-full resize-none overflow-visible whitespace-pre-wrap break-words border-none bg-transparent p-0 outline-none select-text appearance-none"
                    style={textStyle}
        ...
        return (
            <div className="h-full w-full overflow-visible whitespace-pre-wrap break-words bg-transparent p-0" style={textStyle} onWheel={(event) => event.stopPropagation()}>
