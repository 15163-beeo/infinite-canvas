    ...
    return (
        <div className="pointer-events-none absolute bottom-5 z-50 flex justify-center" style={{ left: 300, right: 16 }}>
            ...
            <div ref={wrapRef} className="thin-scrollbar pointer-events-auto flex h-14 max-w-full items-center gap-1 overflow-x-auto rounded-xl border px-2 shadow-lg backdrop-blur [&>*]:shrink-0" style={dockStyle}>
                <ToolbarButton id="tool-select" label="框选/选择" active={canvasTool === "select"} hovered={hovered} activeStyle={activeStyle} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={() => onCanvasToolChange("select")}>
                    <MousePointer2 className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-hand" label="拖动画布" active={canvasTool === "pan"} hovered={hovered} activeStyle={activeStyle} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={() => onCanvasToolChange("pan")}>
...
function toolLabel(id: string) {
    if (id === "tool-select") return "框选/选择";
    if (id === "tool-hand") return "拖动画布";
