...
export default function PromptsPage() {
    ...
    const { query, items: promptItems, tags: promptTags, categories: promptCategoryOptions, total: totalPrompts } = usePromptList({ keyword: titleKeyword, tags: selectedTags, category: selectedCategory, enabled: promptLibraryEnabled });
        ...
        return (
            <div className="flex h-full items-center justify-center bg-background px-6 text-center">
                <div className="max-w-md space-y-4">
                    <div className="text-3xl font-semibold text-stone-950 dark:text-stone-100">提示词库已暂时停用</div>
                    <p className="text-sm leading-6 text-stone-500 dark:text-stone-400">前台入口已经关闭，先保留现有代码，后面需要恢复时直接打开开关即可。</p>
                    <div className="flex justify-center gap-3">
                        <Button type="primary" href="/image">
                            返回生图工作台
                        </Button>
                        <Button href="/assets">打开我的素材</Button>
                    </div>
                </div>
            </div>
        );
