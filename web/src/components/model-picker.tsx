...
export function ModelPicker({ config, value, channelId, onChange, className, fullWidth = false, placeholder = "选择模型", onMissingConfig, excludeModels = [] }: ModelPickerProps) {
    const pickerId = useId();
    const [open, setOpen] = useState(false);
    const excluded = useMemo(() => new Set(excludeModels.map((item) => item.trim().toLowerCase()).filter(Boolean)), [excludeModels]);
    const channelOptions = useMemo(() => {
        ...
        return channels.flatMap((channel) =>
            channel.models
                .filter((model) => !excluded.has(model.trim().toLowerCase()))
                .map((model) => ({ key: `${channel.id}::${model}`, channelId: channel.id, channelName: channel.name, model })),
        );
    }, [config, excluded]);
    const options = useMemo(() => {
        const existing =
            value && !excluded.has(value.trim().toLowerCase()) && !channelOptions.some((item) => item.model === value && (!channelId || item.channelId === channelId))
                ? [{ key: `${channelId || "__current__"}::${value}`, channelId: channelId || "", channelName: "当前", model: value }]
                : [];
        return [...existing, ...channelOptions];
    }, [channelId, channelOptions, excluded, value]);
    const current = value || "";
